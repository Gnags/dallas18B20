...
void DallasTemperature::hugo(OneWire* _oneWire) {
    _wire = _oneWire;
    devices = 0;
    parasite = false;
    bitResolution = 9;
...
void DallasTemperature::begin(void)
{
    ...
    initTries ++; // incr counter for number of tries to find sensors
    ds18B20Idx = 255;
    dallasMs = millis();
...
void DallasTemperature::begin(uint8_t *addrArray, uint8_t offset) {
    // addrArray must be uint8_t addrArray[>=96]
    // offset must be 0 or 1
    
    uint8_t offsetBytes = offset*(6*8); // if offset is 0, result is 0, if offset is 1, result is 48, meaning we are in bus[1] and the first 6 entries (0-47) is reserved to bus[0]
    DeviceAddress deviceAddress;
    ...
    while (_wire->search(deviceAddress))
    {
        if (validAddress(deviceAddress))
        {
            ...
            for(int i=0;i<8;i++) addrArray[offsetBytes + (devices*8) +i] = deviceAddress[i];
...
{
    dallasMs = millis(); // remember when we requested data, so we can give time before reading data
    ds18B20Idx=0; // after request, start reading the first ds18B20 device
    
    _wire->reset();
    _wire->skip();
    _wire->write(STARTCONVO, parasite);

    waitAfterRequest = true;
...
{
    if(ds18B20Idx >= devices) ds18B20Idx = 0;
    else ds18B20Idx++; // shift to next sensor;
    
    dallasMs = millis();
    waitAfterRequest = false;
    ScratchPad scratchPad;
